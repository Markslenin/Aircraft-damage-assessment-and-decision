...
visInfo = check_visualization_toolchain(false);
...
if visInfo.hasSimulation3DBlocks
...
add_block('simulink/Ports & Subsystems/Subsystem', modelName + "/Visualization Interface", ...
    'Position', [1250 180 1540 360]);
add_block('simulink/Signal Routing/Mux', modelName + "/State_Vector_Mux", ...
    'Inputs', '4', ...
    'Position', [500 70 505 210]);
add_block('simulink/Signal Routing/Mux', modelName + "/Visualization_Mode_Mux", ...
    'Inputs', '2', ...
    'Position', [1235 435 1240 505]);
add_block('simulink/User-Defined Functions/Interpreted MATLAB Function', modelName + "/Visualization_Mode_Proxy", ...
    'MATLABFcn', 'visualization_mode_proxy', ...
    'OutputDimensions', '1', ...
    'Position', [1280 440 1400 500]);
...
configureVisualizationInterface(modelName + "/Visualization Interface", visInfo);
...
add_block('simulink/Sinks/Terminator', modelName + "/identifier_uncertainty_sink", ...
    'Position', [1260 570 1280 590]);
...
add_line(modelName, 'Online Damage Identifier/1', 'Visualization_Mode_Mux/1', 'autorouting', 'on');
add_line(modelName, 'Online Damage Identifier/2', 'Visualization_Mode_Mux/2', 'autorouting', 'on');
add_line(modelName, 'Visualization_Mode_Mux/1', 'Visualization_Mode_Proxy/1', 'autorouting', 'on');
add_line(modelName, 'State_Vector_Mux/1', 'Visualization Interface/1', 'autorouting', 'on');
add_line(modelName, 'Visualization_Mode_Proxy/1', 'Visualization Interface/2', 'autorouting', 'on');
add_line(modelName, 'Online Damage Identifier/1', 'Visualization Interface/3', 'autorouting', 'on');
add_line(modelName, 'Online Damage Identifier/2', 'Visualization Interface/4', 'autorouting', 'on');
...
add_line(modelName, 'Online Damage Identifier/3', 'identifier_uncertainty_sink/1', 'autorouting', 'on');
...
add_block('simulink/Signal Routing/Mux', blockPath + "/IdentifierInputMux", ...
    'Inputs', '4', ...
    'Position', [105 88 110 182]);
add_block('simulink/User-Defined Functions/Interpreted MATLAB Function', blockPath + "/IdentifierBridge", ...
    ...
    'OutputDimensions', '6', ...
    'Position', [150 120 290 170]);
add_block('simulink/Signal Routing/Demux', blockPath + "/IdentifierDemux", ...
    'Outputs', '[4 1 1]', ...
    'Position', [320 118 325 186]);
add_block('simulink/Ports & Subsystems/Out1', blockPath + "/eta_hat", ...
    'Position', [355 120 385 134]);
add_block('simulink/Ports & Subsystems/Out1', blockPath + "/confidence", ...
    'Position', [355 155 385 169]);
add_block('simulink/Ports & Subsystems/Out1', blockPath + "/uncertaintyScore", ...
    'Position', [355 190 385 204]);

add_line(blockPath, 'state_summary/1', 'IdentifierInputMux/1', 'autorouting', 'on');
add_line(blockPath, 'control_input/1', 'IdentifierInputMux/2', 'autorouting', 'on');
add_line(blockPath, 'feature_summary/1', 'IdentifierInputMux/3', 'autorouting', 'on');
add_line(blockPath, 'theta_d/1', 'IdentifierInputMux/4', 'autorouting', 'on');
add_line(blockPath, 'IdentifierInputMux/1', 'IdentifierBridge/1', 'autorouting', 'on');
...
add_line(blockPath, 'IdentifierDemux/3', 'uncertaintyScore/1', 'autorouting', 'on');
end

function configureVisualizationInterface(blockPath, visInfo)
open_system(blockPath);
Simulink.SubSystem.deleteContents(blockPath);

add_block('simulink/Ports & Subsystems/In1', blockPath + "/state_vector", ...
    'Position', [25 38 55 52]);
add_block('simulink/Ports & Subsystems/In1', blockPath + "/decision_mode", ...
    'Position', [25 88 55 102]);
add_block('simulink/Ports & Subsystems/In1', blockPath + "/eta_hat", ...
    'Position', [25 138 55 152]);
add_block('simulink/Ports & Subsystems/In1', blockPath + "/confidence", ...
    'Position', [25 188 55 202]);
add_block('simulink/Sinks/Terminator', blockPath + "/decision_mode_sink", ...
    'Position', [110 85 130 105]);
add_block('simulink/Sinks/Terminator', blockPath + "/eta_sink", ...
    'Position', [110 135 130 155]);
add_block('simulink/Sinks/Terminator', blockPath + "/confidence_sink", ...
    'Position', [110 185 130 205]);

if strcmpi(visInfo.recommendedVisualizationMode, 'unreal_3d') && visInfo.hasSimulation3DBlocks
    add_block('simulink/Signal Routing/Demux', blockPath + "/StateDemux", ...
        'Outputs', '[3 3 3 3]', ...
        'Position', [95 28 100 172]);
    add_block('sim3dlib/Simulation 3D Scene Configuration', blockPath + "/Scene Configuration", ...
        'Position', [155 15 335 65]);
    set_param(blockPath + "/Scene Configuration", ...
        'SceneDesc', 'Straight road', ...
        'EnableWindow', 'off');
    add_block('sim3dlib/Simulation 3D Static Mesh Actor', blockPath + "/Aircraft Proxy", ...
        'Position', [210 95 380 185]);
    set_param(blockPath + "/Aircraft Proxy", ...
        'StaticMeshActorType', 'Cone', ...
        'InitialScale', '[1 1 1]');
    add_block('simulink/Sources/Constant', blockPath + "/scale_xyz", ...
        'Value', '[1 1 1]', ...
        'Position', [145 205 205 235]);
    add_block('simulink/Sinks/Terminator', blockPath + "/mesh_out_1", ...
        'Position', [420 105 440 125]);
    add_block('simulink/Sinks/Terminator', blockPath + "/mesh_out_2", ...
        'Position', [420 135 440 155]);
    add_block('simulink/Sinks/Terminator', blockPath + "/mesh_out_3", ...
        'Position', [420 165 440 185]);
    add_line(blockPath, 'state_vector/1', 'StateDemux/1', 'autorouting', 'on');
    add_line(blockPath, 'StateDemux/1', 'Aircraft Proxy/1', 'autorouting', 'on');
    add_line(blockPath, 'StateDemux/3', 'Aircraft Proxy/2', 'autorouting', 'on');
    add_line(blockPath, 'scale_xyz/1', 'Aircraft Proxy/3', 'autorouting', 'on');
    add_line(blockPath, 'Aircraft Proxy/1', 'mesh_out_1/1', 'autorouting', 'on');
    add_line(blockPath, 'Aircraft Proxy/2', 'mesh_out_2/1', 'autorouting', 'on');
    add_line(blockPath, 'Aircraft Proxy/3', 'mesh_out_3/1', 'autorouting', 'on');
else
    add_block('simulink/Signal Routing/Demux', blockPath + "/StateDemux", ...
        'Outputs', '[3 3 3 3]', ...
        'Position', [95 35 100 170]);
    add_block('simulink/Sinks/Terminator', blockPath + "/position_sink", ...
        'Position', [155 30 175 50]);
    add_block('simulink/Sinks/Terminator', blockPath + "/velocity_sink", ...
        'Position', [155 70 175 90]);
    add_block('simulink/Sinks/Terminator', blockPath + "/attitude_sink", ...
        'Position', [155 110 175 130]);
    add_line(blockPath, 'state_vector/1', 'StateDemux/1', 'autorouting', 'on');
    add_line(blockPath, 'StateDemux/1', 'position_sink/1', 'autorouting', 'on');
    add_line(blockPath, 'StateDemux/2', 'velocity_sink/1', 'autorouting', 'on');
    add_line(blockPath, 'StateDemux/3', 'attitude_sink/1', 'autorouting', 'on');
end

add_line(blockPath, 'decision_mode/1', 'decision_mode_sink/1', 'autorouting', 'on');
add_line(blockPath, 'eta_hat/1', 'eta_sink/1', 'autorouting', 'on');
add_line(blockPath, 'confidence/1', 'confidence_sink/1', 'autorouting', 'on');
end
