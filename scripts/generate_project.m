...
add_line(modelName, 'Damage_Injection/3', 'Actuator_Propulsion/3', 'autorouting', 'on');
...
add_block('aerolibatmos/ISA Atmosphere Model', blockPath + "/Atmosphere", ...
    'Position', [110 20 250 80]);
add_block('aerolibgravity/WGS84 Gravity Model', blockPath + "/Gravity", ...
    'Position', [110 90 250 150]);
add_block('aerolibwind/Discrete Wind Gust Model', blockPath + "/Wind", ...
    'Position', [110 165 250 225]);
add_block('simulink/Signal Routing/Bus Creator', blockPath + "/BusCreator", ...
    'Inputs', '3', ...
    'Position', [390 108 410 172]);

add_line(blockPath, 'altitude_m/1', 'Atmosphere/1', 'autorouting', 'on');
add_line(blockPath, 'latitude_rad/1', 'Gravity/1', 'autorouting', 'on');
add_line(blockPath, 'altitude_m/1', 'Gravity/2', 'autorouting', 'on');
...
add_line(blockPath, 'eta_ctrl/1', 'InputMux/3', 'autorouting', 'on');
...
add_block('simulink/Sources/Constant', blockPath + "/u_cmd_placeholder", ...
    'Value', 'P.control.trim', ...
    'Position', [210 45 290 75]);
...
add_line(blockPath, 'u_cmd_placeholder/1', 'u_cmd_out/1', 'autorouting', 'on');
end
