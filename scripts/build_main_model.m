function build_main_model(rootDir)
%BUILD_MAIN_MODEL Construct the main_damaged_aircraft Simulink model.
%   build_main_model(rootDir) deletes any existing copy of the model under
%   models/main_damaged_aircraft.slx and rebuilds the closed-loop diagram
%   from scratch: 6DoF airframe, environment bus, actuator/propulsion,
%   damage injection, decision logic, online identifier bridge, sensor bus,
%   and the visualization interface.
%
%   This was previously inlined inside scripts/generate_project.m; it has
%   been split out so generate_project.m only handles MATLAB Project plumbing
%   and so the model can be regenerated independently if its wiring is
%   reset by mistake.
%
%   Note: the build_* / configure_* helpers below talk to the Simulink API
%   directly. Running this function therefore requires Simulink, the
%   Aerospace Blockset, and (optionally, for the 3D visualization branch)
%   the Simulation 3D library.
