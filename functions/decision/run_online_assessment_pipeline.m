function result = run_online_assessment_pipeline(theta_d, identifierModel, identifierConfig, mode, scenarioInfo, cachedSample)
%RUN_ONLINE_ASSESSMENT_PIPELINE Run oracle or identified assessment chain.
%
%   When cachedSample is supplied (typically a row of identifierDataset.samples
%   that already carries ctrlMetrics, trimInfo, decisionOutput, residual and
%   featureModeReadyData), simulate_identifier_timeseries is skipped. Batch
%   evaluation paths that previously paid for two redundant forward
%   simulations per sample (oracle + identified) become a single pass.
